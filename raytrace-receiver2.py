...
def stop(ren, appendFilter, srf1, srf2, scene):
    ...
    for idx in range(srf1['raypoints'].GetNumberOfPoints()):
        ...
        pointRaySurf2 = ray.n2l(ray.l2n(pointSurf1) + ray.RayCastLength*ray.l2n(normalsurf1))
...
def asphere(srf):
    ...
    if 'raypoints' in srf:
        ...
        transform.PostMultiply()
        if 'center' in srf:
            transform.Translate([-1*p for p in srf['center']])
        if 'rotateWXYZ' in srf:
            transform.RotateWXYZ(-1*srf['rotateWXYZ'][0], srf['rotateWXYZ'][1:])
        pgt=vtk.vtkTransformPolyDataFilter()
        pgt.SetOutputPointsPrecision(vtk.vtkAlgorithm.DOUBLE_PRECISION)
        pgt.SetTransform(transform)
...
def flatcircle(srf):
    ...
    extrude.Update()
    # It would be best to rotate it by 360/res, so simple rays don't hit eges and low res can be used    
...
def objectsource(srf1, srf2, ratio=0.8):
    ...
    for sp in sourcepoints:
        for tp in targetpoints:
            ...
            object_normalvectors.InsertNextTuple(ray.n2l(vec/np.linalg.norm( vec  ) ))
...
def pointsource(srf, simple=True):
    if simple:
        ...
        for rr  in res:
            ...
            theta = thetax*thetay/np.sqrt((thetay*np.cos(phi))**2+(thetax*np.sin(phi))**2)
...
def shape(ren, appendFilter, srf, addActor):
    if srf['shape'] == 'sphere':
...
def trace(ren, appendFilter, srf1, srf2, addActor=True):
    ...
    for idx in range(srf1['raypoints'].GetNumberOfPoints()):
        ...
        if ray.isHit(obbsurf2, pointSurf1, pointRaySurf2):
            ...
            vecInc2 = ray.l2n(pointRaySurf2) - ray.l2n(pointSurf1)
            vecInc = ray.n2l(vecInc2/np.linalg.norm(vecInc2))
...
def run(surfaces, project, Directory, scene, refine=True, plot=True):    
    ...
    for tri in traceit:
        ...
        if tri == 0:
            ...
            renWin.Render()
            print 'Tracing ',str(tri-1),' and ',tri
        else:
            if refine == True: # If refine, shape again, trace again
...
if __name__ == "__main__":
    ...
    #del surfaces[3:5]
    project = 'receiverB'
    Directory = '/home/jaap/Innoluce/Spyder/optics/lidar/'
    #del surfaces[2:5]
